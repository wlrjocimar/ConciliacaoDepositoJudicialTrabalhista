 ...
 * @author PC_LENOVO
 */
public class DesconciliacaoEstoqueDAO implements CrudDAO<Desconciliacao> {
    ...
    public List<Desconciliacao> buscar() throws ErroSistema {
        ...
        try {
            Connection con = ConnectionFactory.conectar("retab");
            String sql = "SELECT * FROM retab.tb_desconciliacao_djo_paj where ((NOME_TRATAMENTO = '' OR NOME_TRATAMENTO is null) AND (AVOCADO = '' OR AVOCADO IS NULL OR( AVOCADO = 'SIM' AND FUNCIONARIO_ATUAL = '" + usuario.getChave() + "' ) ) AND(DATA_ENTRADA_BD IS NULL) AND (SITUACAO = '' OR SITUACAO IS NULL) AND(ABS(VALOR_DESCONCILIACAO) < 50000)) ORDER BY DATA_DESCONCILIACAO ASC,VALOR_DESCONCILIACAO DESC LIMIT 1";
