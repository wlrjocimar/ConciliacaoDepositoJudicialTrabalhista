 ...
 * @author PC_LENOVO
 */
public class DesconciliacaoNovosAcimaQuantidadeDAO implements CrudDAO<Desconciliacao> {
        ...
        try {
            ...
            if (entidade.getDataPrimeiroTratamento()== null) {
                 sql = "UPDATE tb_desconciliacao_djo_paj set NPJ=?, CONTA_CONTROLE=? , CONTA_DEPOSITARIA=? , SALDO_CONTA_CONTROLE=? , SALDO_DEPOSITO=?"
                        ...
                        + " , NOME_TRATAMENTO=? , AVOCADO=?, DATA_AVOCACAO=?, DATA_DESCONCILIACAO=?, OBS_LIVRE = ?, DATA_PRIMEIRO_TRATAMENTO = ?, TRATADO_PRAZO = ?  where CODIGO_DESCONCILIACAO=?";
    ...
    public List<Desconciliacao> buscar() throws ErroSistema {
        ...
        try {
            Connection con = ConnectionFactory.conectar("retab");
            String sql = "SELECT * FROM retab.tb_desconciliacao_djo_paj where  (AVOCADO = '' OR AVOCADO IS NULL OR( AVOCADO = 'SIM' AND FUNCIONARIO_ATUAL =  '" + usuario.getChave() + "' ) ) AND(DATA_ENTRADA_BD IS NOT NULL) AND (SITUACAO = '' OR SITUACAO IS NULL OR SITUACAO ='INEDITO TRATADO' ) AND((ABS(VALOR_DESCONCILIACAO) >= 50000))";
...
//            stmt.setString(14, entidade.getAvocado());
    ...
    public Boolean avaliarParaSalvar(Desconciliacao entidade) {
         String status = entidade.getSituacao();
         String complemento = entidade.getNomeTratamento();
        
        
                if (status.equals("EM TRATAMENTO") && complemento.equals("")){
                ...
                }else if(status.equals("INCONSISTENCIA") && complemento.equals("")){
