...
public class DesconciliacaoRetornoAgenciaAcimaDAO implements CrudDAO<Desconciliacao> {
    ...
    public List<Desconciliacao> buscar() throws ErroSistema {
        ...
        try {
            Connection con = ConnectionFactory.conectar("retab");
            String sql = "SELECT * FROM retab.tb_desconciliacao_djo_paj where ((SITUACAO = 'RETORNADO AGENCIA' ) AND (AVOCADO = '' OR AVOCADO IS NULL OR ( AVOCADO = 'SIM' AND FUNCIONARIO_ATUAL =  '" + usuario.getChave() + "' )) AND(abs(VALOR_DESCONCILIACAO) >= 50000))  ORDER BY VALOR_DESCONCILIACAO DESC LIMIT 1;";
