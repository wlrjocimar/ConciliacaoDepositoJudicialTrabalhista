 ...
 * @author PC_LENOVO
 */
public class DesconciliacaoNaoTratadaDAO implements CrudDAO<Desconciliacao> {
    ...
    public List<Desconciliacao> buscar() throws ErroSistema {
        try {
            Connection con = ConnectionFactory.conectar("rejud");
            String sql = "SELECT * FROM tb_desconciliacao_djo_paj where (SITUACAO IS NULL OR SITUACAO= '') and(DATA_PRIMEIRO_TRATAMENTO IS NULL OR DATA_PRIMEIRO_TRATAMENTO = '') order by DIAS_DESCONCILIADO DESC";
