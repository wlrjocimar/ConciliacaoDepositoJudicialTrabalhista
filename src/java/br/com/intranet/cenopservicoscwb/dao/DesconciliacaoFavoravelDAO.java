...
public class DesconciliacaoFavoravelDAO implements CrudDAO<Desconciliacao> {
    ...
    public List<Desconciliacao> buscar() throws ErroSistema {
        ...
        try {
            Connection con = ConnectionFactory.conectar("rejud");
            String sql = "SELECT * FROM rejud.tb_desconciliacao_djo_paj where (SITUACAO = 'EM TRATAMENTO' OR SITUACAO LIKE '%' 'FAVO' '%') AND NOME_TRATAMENTO  LIKE '%' 'FAVO' '%'";
         ...
         try {
           ...
           stmt.execute();
            ...
            stmt.execute();
             ...
             sql = "update rejud.tb_desconciliacao_djo_paj  as t1 set SITUACAO = 'REGULARIZADO', NOME_TRATAMENTO = 'ANOTAÇÃO AUTOMÁTICA',DATA_SITUACAO = curdate() WHERE  exists (select * from rejud.tb_tmp_concluidos as t2 where t1.NPJ=t2.NPJ and t1.CONTA_DEPOSITARIA=t2.CONTA_DEPOSITARIA and (t1.SITUACAO is null or t1.SITUACAO = ''))";
             stmt = con.prepareStatement(sql);
             stmt.execute();
            ...
            stmt.execute();
             ...
             sql = "UPDATE rejud.tb_desconciliacao_djo_paj  as t1 set TRATADO_PRAZO = 'SIM' where DIAS_DESCONCILIADO  <=10 and DIAS_DESCONCILIADO>=0 AND (TRATADO_PRAZO IS NULL OR TRATADO_PRAZO = 'NAO') AND DATA_PRIMEIRO_TRATAMENTO IS NOT NULL AND SITUACAO = 'REGULARIZADO' AND NOME_TRATAMENTO = 'ANOTAÇÃO AUTOMÁTICA'";
             stmt = con.prepareStatement(sql);
             stmt.execute();
