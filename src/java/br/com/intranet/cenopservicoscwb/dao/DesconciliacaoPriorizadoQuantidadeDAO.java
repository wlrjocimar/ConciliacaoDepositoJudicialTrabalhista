...
public class DesconciliacaoPriorizadoQuantidadeDAO implements CrudDAO<Desconciliacao> {
    ...
    public List<Desconciliacao> buscar() throws ErroSistema {
        ...
        try {
            Connection con = ConnectionFactory.conectar("retab");
            String sql = "SELECT * FROM retab.tb_desconciliacao_djo_paj where  (AVOCADO = '' OR AVOCADO IS NULL OR( AVOCADO = 'SIM' AND FUNCIONARIO_ATUAL =  '" + usuario.getChave() + "' ) )  AND (SITUACAO = 'PRIORIZADO' ) ORDER BY ABS(VALOR_DESCONCILIACAO)DESC";
