...
public class DesconciliacaoResgatesMultiplosQuantidadeDAO implements CrudDAO<Desconciliacao> {
    ...
    public List<Desconciliacao> buscar() throws ErroSistema {
        ...
        try {
            Connection con = ConnectionFactory.conectar("retab");
            String sql = "SELECT * FROM retab.tb_desconciliacao_djo_paj where ((SITUACAO = 'EM TRATAMENTO' AND NOME_TRATAMENTO = 'EM TRATAMENTO - DESCONCILIACAO ACIMA COM RESGATES ABAIXO 50 MIL') AND (AVOCADO = '' OR AVOCADO IS NULL OR ( AVOCADO = 'SIM' AND FUNCIONARIO_ATUAL =  '" + usuario.getChave() + "' )) )  ORDER BY VALOR_DESCONCILIACAO DESC,DATA_DESCONCILIACAO ASC";
